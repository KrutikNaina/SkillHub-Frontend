...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-purple-100 to-pink-100">
      {/* Sidebar */}
      <aside className="w-64 bg-white shadow-xl rounded-r-3xl p-6 hidden md:flex flex-col justify-between fixed top-0 left-0 h-screen z-50">
        <div>
        ...
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Here’s your skill-building dashboard.
            </p>
          </div>
          <img
            src={
        ...
        <div className="mt-10 bg-white p-6 rounded-2xl shadow-md flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Keep the fire alive! Log daily progress to maintain your streak.
            </p>
          </div>
        </div>
          ...
          <ul className="bg-white rounded-xl p-6 shadow-md space-y-4">
            {recentSkills.length > 0 ? (
              recentSkills.map((skill, idx) => (
                <li key={skill._id || idx} className="border-b pb-2">
                  ⭐ {skill.title} –{" "}
                  {new Date(skill.createdAt).toLocaleDateString()}
                </li>
              ))
            ) : (
              <li className="border-b pb-2">⭐ No recent skills found</li>
            )}
          </ul>
        </div>
      </main>
